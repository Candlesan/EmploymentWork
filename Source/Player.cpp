...
Player::Player()
{
	...
	// モデル初期化
	player = std::make_shared<Model>(device, "Data/Model/unitychan/unitychan.glb");
...
void Player::UpdateAnimations(float elapsedTime)
{
	...
	case Player::State::Walk:
		animationLoop = true;
		useRootMotion = false;
		newAnimationIndex = player->GetAnimationIndex("WalkForwardInPlace");
	...
	case Player::State::Run:
		...
		newAnimationIndex = player->GetAnimationIndex("RunForward");
	...
	if (animationIndex >= 0)
	{
		...
		{
			// ルートモーションノード番号取得
			const int rootMotionNodeIndex = player->GetNodeIndex("Character1_Hips"); // モデルに合わせて変えてくれ
		...
		else if (useRootMotionEx)
		{
			...
			const int rootMotionNodeIndex = player->GetNodeIndex("Character1_Hips");
