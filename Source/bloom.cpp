...
{
	OutputDebugStringA(("Bloom size: " + std::to_string(width) + "x" + std::to_string(height) + "\n").c_str());
...
void bloom::make(ID3D11DeviceContext* immediate_context, ID3D11ShaderResourceView* color_map, RenderState* render_state)
{
	...
	// サンプラーステートのキャッシュを追加
	ID3D11SamplerState* cached_samplers[5]{};
	immediate_context->PSGetSamplers(0, 5, cached_samplers);
	...
	{
		render_state->GetSamplerState(SamplerState::PointWrap),      // [0] POINT
		render_state->GetSamplerState(SamplerState::LinearWrap),     // [1] LINEAR
		render_state->GetSamplerState(SamplerState::LinearWrap),     // [2] ANISOTROPIC (代用)
		render_state->GetSamplerState(SamplerState::LinearClamp),    // [3] LINEAR_BORDER_BLACK
		render_state->GetSamplerState(SamplerState::LinearClamp),    // [4] LINEAR_BORDER_WHITE	}
	};
	immediate_context->PSSetSamplers(0, _countof(samplers), samplers);
	...
	// サンプラーステートを解除
	immediate_context->PSSetSamplers(0, 5, cached_samplers);
	for (auto& sampler : cached_samplers)
	{
...
void bloom::set_constant_buffer(ID3D11DeviceContext* immediate_context)
{
	bloom_constants data{};
	data.bloom_extraction_threshold = bloom_extraction_threshold;
	data.bloom_intensity = bloom_intensity;
